...
var sampleAdrs = new List<ArchitectureDecision>
{
    new()
    {
        ...
        Content = "In January 2024, we decided to adopt gRPC over REST for internal microservices communication. " +
        "Key reasons: 1) Binary protocol (Protocol Buffers) provides 10x better performance than JSON, 2) Strong typing with .proto contracts reduces integration bugs, 3) Native support for bidirectional streaming enables real-time data flows, 4) HTTP/2 multiplexing reduces connection overhead. Trade-offs accepted: steeper learning curve and reduced browser compatibility (acceptable for internal services)."
    },
    new()
    {
        ...
        "Reasons: ACID compliance required for financial transactions, mature tooling ecosystem, excellent JSON support for semi-structured data, and lower operational costs. We use MongoDB only for specific document-heavy workloads."
    },
    new()
    {
        DocumentId = Guid.Parse("33333333-3333-3333-3333-333333333333"),
        Title = "ADR-003: Event-Driven Architecture with Kafka",
        Content = "In February 2024, we adopted Apache Kafka for event-driven communication between bounded contexts. " +
        "This decouples services, enables event sourcing, and provides replay capability for debugging. RabbitMQ was rejected due to limited horizontal scaling."
    }
};
